...
exports.compute = function(values) {
  ...
  for (var i = 0; i < computer.output.length; i += 3) {
    if (computer.output[i + 2] === 3) {
      paddlePos = [computer.output[i], computer.output[i + 1]];
    } else if (computer.output[i + 2] === 4) {
      ballPos = [computer.output[i], computer.output[i + 1]];
    }
  }
  computer.output = [];
  ...
  while (!computer.isHalted) {
    ...
    for (var i = 0; i < computer.output.length; i += 3) {
      if (computer.output[i + 2] === 4) {
        ballPos = [computer.output[i], computer.output[i + 1]];
      }
      if (computer.output[i] === -1 && computer.output[i + 1] === 0) {
        score = computer.output[i + 2];
      }
    }
